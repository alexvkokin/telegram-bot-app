...
`'message'` - Выше были описаны способы обработки события по клику на кнопке в боте. Данный атрибут содержит название типа сообщения. Если от бота приходит сообщение данного типа, то будет выполняться метод `'actionMessage'` соответствующего класса
